...
pub async fn build_app_state(
    ...
    let (app_db, repos) =
        db::create_db(database_url, default_currency_code, invoice_config).await?;
...
mod tests {
    ...
    async fn test_health_check_connected() {
        let (state, _) = build_app_state(&test_db_url(), "idr", Default::default())
            .await
            .unwrap();
    ...
    async fn test_build_app_state() {
        let (state, db) = build_app_state(&test_db_url(), "idr", Default::default())
            .await
            .unwrap();
